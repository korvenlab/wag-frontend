...
export function TeamManagementPage() {
  ...
  const loadTeam = useCallback(async () => {
    ...
    try {
      ...
      setHasPlan(!!data.multi_barber_plan);
    } catch {
  ...
  useEffect(() => {
    ...
    if (params.get("checkout") === "multi_barber_success") {
      void refreshProfile();
      void loadTeam();
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      const res = await fetch(`${backendUrl}/api/stripe/create-multi-barber-checkout-session`, {
        method: "POST",
        headers: {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] relative">
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="fixed top-0 right-0 w-[500px] h-[500px] bg-green-50/50 blur-[120px] rounded-full -z-10"
      />
      ...
      <header className="border-b border-slate-100 bg-white/80 backdrop-blur-md sticky top-0 z-30">
        <motion.div
          ...
          className="max-w-4xl mx-auto px-6 py-4 flex items-center justify-between gap-4"
        >
          <Button
            variant="ghost"
        ...
        <div className="relative">
          <div
            className={`space-y-6 transition-all duration-300 ${
              showPaywall ? "blur-md pointer-events-none select-none" : ""
            }`}
          >
              ...
              <Card className="max-w-md w-full rounded-[32px] border-none shadow-2xl bg-white/95 backdrop-blur-sm p-8 text-center space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-slate-500 font-medium leading-relaxed">
                    Atenda com mais de 1 profissional usando o mesmo número de WhatsApp. A IA
                    apresenta sua equipe, o cliente escolhe o barbeiro e o convite cai direto no
                    Google Agenda de cada um.
                  </p>
                </div>
                <ul className="text-left text-sm text-slate-600 space-y-2 font-medium">
                  <li className="flex gap-2">
                    <span className="text-[#64b34d]">✓</span> Vários profissionais, um WhatsApp
                  </li>
                  <li className="flex gap-2">
                    <span className="text-[#64b34d]">✓</span> Convites automáticos no Google Agenda
                  </li>
                  <li className="flex gap-2">
                    <span className="text-[#64b34d]">✓</span> Agenda centralizada no dono da barbearia
                  </li>
                </ul>
                <Button
                ...
                >
                  {upgrading ? (
                    ...
                    "Fazer Upgrade para Multi-Barbeiro"
                  )}
                </Button>
              </Card>
            </motion.div>
